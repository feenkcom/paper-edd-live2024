...
Although some tests encode interesting scenarios that answer certain questions about how the system works, most tests tend to be uninteresting. \ac{I would not say that tests are uninteresting, but that usually usage is limited to running the test to see if it passes, or reading the code of the test}
...
Although technically examples constitute just a tiny \ac{not sure we need to say necessarily "tiny"} modification to test methods, their impact is potentially ground-breaking.
We show
\begin{inparaenum}[(i)]
...
\del{In practice, you have to ``guess first'' to imagine an API to express the interactions needed to exercise and test functionality you want to implement.}
\tg{Sometimes you know upfront how to create the setup and what the desired outcome is. But oftentimes you do not know upfront what the right assertion should be. In this situation you have to ``guess first'' to write the assertion. And even when you do know the assertion upfront, you have to ``guess first'' to imagine an API to express the interactions needed to exercise and test functionality you want to implement.}
...
A red test is useful, as long as it brings you to a debugger that lets you explore why the test has failed, but a green test is uninteresting \del{and useless}.
...
\del{In \autoref{fig:Memory} we see an extract of tutorial pages explaining how to build a graphical user interface using examples of a ``Memory'' game.\footnote{\url{https://en.wikipedia.org/wiki/Concentration_(card_game)}} The page contains a live, embedded example of a partially played Memory game with two cards turned over.}

\tg{In \autoref{fig:LudoBook} we see an extract of a page describing the Ludo game.
The page section describes the rules of the game for new a live, embedded example of the initial move of a player. The picture is created by a view on a move object produced by the \st{moveInitial} example method from the \st{GtLudoRecordingGameExamples} class.}
