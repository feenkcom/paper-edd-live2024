...
We start (\autoref{fig:exampleCreationA}, leftmost pane) by writing a snippet that creates a new \lst{ConcretePrice} object with \st{100 euros} as its \st{money} value.
...
Evaluating this snippet and inspecting the result (\autoref{fig:exampleCreationB}, right pane) shows that this gives us the \st{ConcretePrice} object that we expect.
